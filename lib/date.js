...
// ジェネレートする際の開始日付
const minDateGenerate = '20210101'
 ...
 */
export function dateString(str, detail = false) {
  ...
  if (detail) {
    return `${year}年${month}月${day}日(${weekString}) ${hours}:${minutes}`
  } else {
 ...
 * @return obj {yyyymmdd, yyyymmdd}
 */
export function getAllTerm(isAll = false) {
  const targetMinDate = isAll ? minDate : minDateGenerate
  let endDay = ''
  const dateArray = []
 ...
 * @return str yyyymmdd or yyyymm
 */
export function getBeforeDate(str, beforeDay = 1) {
  if (String(str).length === 8) {
    // スラッシュ付きに変換して日付を取得
    const now = new Date(addSlash(str))
    now.setDate(now.getDate() - beforeDay)
    ...
    // スラッシュ付きに変換して日付を取得
    const now = new Date(addSlash(str))
    now.setMonth(now.getMonth() - beforeDay)
    const year = now.getFullYear()
    const month = toDoubleDigits(now.getMonth() + 1)
